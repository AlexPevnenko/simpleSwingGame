...
public class GameWindow extends JFrame {
    ...
    private static GameWindow gameWindow; // объявили переменную
    ...
    public static void main(String[] args) throws IOException // вынос исключения в сигнатуру метода
    {
        background = ImageIO.read( GameWindow.class.getResourceAsStream( "background.png" ) );
        gameOver = ImageIO.read( GameWindow.class.getResourceAsStream( "game_over.png" ) );
        drop= ImageIO.read( GameWindow.class.getResourceAsStream( "drop.png" ) );
        gameWindow = new GameWindow(); //создали объект окна
        ...
        lastFrameTime =System.nanoTime();//возращает текущее время на нс
    ...
    private static void onRepaid( Graphics g )
    {
        ...
        g.drawImage( background,0,0 ,null);//рисуем картинку
        g.drawImage( drop,(int)dropLeft,(int)dropTop,null );
        if(dropTop > gameWindow.getHeight()) g.drawImage( gameOver,300,120,null );
    }

    //рисовать можем на панелях

    private static class GameField extends JPanel
    {
        //диначиеское замещемние методов
        @Override
        protected void paintComponent (Graphics g)
        {
         super.paintComponent( g );
         onRepaid( g );
         repaint(  ); // что бы отрисока была часто
        }
    }
}
